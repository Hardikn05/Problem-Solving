class Solution {
public:
bool f(int row, int col, vector<vector<int>> &vis, vector<vector<int>> &grid, int n, int m){
    vis[row][col] = 1;
    if(row==n-1 && col==m-1) return true;

    if(grid[row][col]==1 && col-1>=0 && (grid[row][col-1]==4 || grid[row][col-1]==6 || grid[row][col-1]==1) && vis[row][col-1]==0){
        if(f(row, col-1, vis, grid, n, m)) return true;
    }
    ...
    if(grid[row][col]==2 && row+1<n && (grid[row+1][col]==5 || grid[row+1][col]==6 || grid[row+1][col]==2) && vis[row+1][col]==0){
        if(f(row+1, col, vis, grid, n, m)) return true;
    }
